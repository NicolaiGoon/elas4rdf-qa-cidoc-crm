...
class AnswerTypePrediction:
    ...
    def classify_category(self, q):
        input_ids = torch.tensor(
            self.category_tokenizer.encode(q, add_special_tokens=True)
        ).unsqueeze(0)
        with torch.no_grad():
    ...
    def classify_resource(self, q):
        input_ids = torch.tensor(
            self.resource_tokenizer.encode(q, add_special_tokens=True)
        ).unsqueeze(
            0
        )  # Batch size 1
        with torch.no_grad():
        ...
        if initial_top != {}:
            norm[initial_top_index] = (
                norm[initial_top_index] + int(initial_top["level"]) / 6
            )
            ...
            for c in initial_top_children:
                if c in self.label_to_id:
                    norm[int(self.label_to_id[c])] = (
                        norm[int(self.label_to_id[c])]
                        + int(self.hierarchy[c]["level"]) / 6
                    )
